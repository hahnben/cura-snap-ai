...
/**
 * Managed worker instance for processing transcription-only jobs in a worker pool
 * Handles transcription jobs without SOAP note generation for user review workflow
 */
@Slf4j
public class ManagedTranscriptWorker {
    ...
    public ManagedTranscriptWorker(String workerId,
        ...
        workerHealthService.registerWorker(workerId, "transcription_only");
        log.info("ManagedTranscriptWorker {} initialized", workerId);
    }
    ...
    public void processJobs() {
        ...
        try {
            ...
            Optional<JobData> nextJob = jobService.getNextJobFromQueue("transcription_only");

            if (nextJob.isPresent()) {
                processTranscriptJob(nextJob.get());
                resetFailureCounter();
            }
     ...
     */
    private void processTranscriptJob(JobData jobData) {
        String jobId = jobData.getJobId();
        String userId = jobData.getUserId();
        Instant jobStartTime = Instant.now();

        log.info("Worker {} processing transcription job {} for user {}", workerId, jobId, userId);
        ...
        try {
            ...
            // Transcribe audio to text (only step - no SOAP generation)
            ...
            // Update job with transcript result (no SOAP note generation)
            ...
            result.put("transcriptionOnly", true); // Flag to indicate transcription-only processing
            ...
            if (updated) {
                ...
                log.info("Worker {} successfully completed transcription job {} for user {} in {}ms",
            ...
            failJob(jobId, "Transcription failed: " + e.getMessage(), "transcription_error", e);

        } catch (Exception e) {
            log.error("Worker {} - Unexpected error processing transcription job {}: {}", workerId, jobId, e.getMessage(), e);
            long processingTime = Duration.between(jobStartTime, Instant.now()).toMillis();
            workerHealthService.recordJobProcessing(workerId, false, processingTime);
            failJob(jobId, "Processing failed: " + e.getMessage(), "unexpected_error", e);
        }
    }
    ...
    private void failJob(String jobId, String errorMessage, String errorType) {
        failJob(jobId, errorMessage, errorType, null);
    }

    /**
     * Handle job failure with adaptive intelligent retry including exception details
     */
    private void failJob(String jobId, String errorMessage, String errorType, Throwable error) {
        try {
